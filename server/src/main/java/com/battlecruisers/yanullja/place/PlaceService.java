...
import com.battlecruisers.yanullja.coupon.CouponService;
...
//        }
        ...
        List<RoomQueryDto> roomQueryDto = roomList.stream()
                .map(room -> {
                    ...
                    Integer maxAvailableRoomCount
                            = reservationService.getMaxAvailableRoomCount(reservations, room, checkInDate, checkOutDate);
                    return RoomQueryDto.from(room, checkInDate, checkOutDate, maxAvailableRoomCount);
                })
                .collect(Collectors.toList());
    ...
    public Page<PlaceQueryDto> searchPlaces(Pageable pageable, SearchConditionDto searchConditionDto) {
        ...
        for (Room room : roomReservationMap.keySet()) {
            Integer maxAvailableRoomCount = reservationService.getMaxAvailableRoomCount(roomReservationMap.get(room), room,
        ...
        placeList = placeList.stream()
                .filter(place -> {
                    List<Room> roomList = place.getRoomList();
                    for (Room room : roomList) {
                        Integer maxAvailableRoomCount = maxAvailableRoomCountMap.get(room);
                        if (maxAvailableRoomCount != null && maxAvailableRoomCount > 0) return true;
                    }
                    return false;
                })
                .collect(Collectors.toList());
